package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class NewPetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener
   {
      ...
      private static const const_1263:int = 18;
      ...
      override public function init() : Boolean
      {
         ...
         this.var_455 = this.getPetTypeIndexFromProduct(_loc2_.localizationId);
         if(this.var_455 < 8)
         {
            return false;
         }
         this.var_2118 = _loc2_.localizationId;
         this.updateAvailableBreeds(_loc2_.localizationId);
         if(this.var_195 != null && this.var_195.length > 0)
         {
            this.var_329 = this.var_195[0];
         }
         else
         {
            this.var_329 = -1;
         }
         this._offers.add(this.var_455,_loc2_);
      ...
      private function initializeBreedSelection() : void
      {
         ...
         if(this.var_195 != null)
         {
            _loc1_ = [];
            _loc2_ = 0;
            while(_loc2_ < const_1263 && _loc2_ < this.var_195.length)
            {
               _loc3_ = (page.viewer.catalog as HabboCatalog).roomEngine.getPetColor(this.var_455,this.var_195[_loc2_]);
               if(_loc3_ != null)
               {
      ...
      private function selectedBreed(param1:int) : void
      {
         if(this.var_195 != null && this.var_195.length > 0)
         {
            ...
            this.var_329 = this.var_195[param1];
            this.updateImage();
         }
      }
      ...
      private function getPetLocalization() : String
      {
         ...
         if(page != null && false)
         {
            ...
            {
               return _loc1_.localization.getKey(this.getRaceLocalizationKey(this.var_455,this.var_329),this.getRaceLocalizationKey(this.var_455,this.var_329));
            }
         }
         return this.getRaceLocalizationKey(this.var_455,this.var_329);
      }
      ...
      private function getPurchaseParameters() : String
      {
         ...
         var p:String = this._name + String.fromCharCode(10) + this.var_329 + String.fromCharCode(10) + this.addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
